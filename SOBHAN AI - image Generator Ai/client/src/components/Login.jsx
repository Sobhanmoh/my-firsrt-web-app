  ...
  return (
    ...
    className='fixed top-0 left-0 right-0 bottom-0 z-10 backdrop-blur-sm bg-black/30 flex justify-center items-center'>
     ...
     className='relative bg-white p-10 rounded-xl text-slate-500'>
        <h1 
        className='text-center text-2xl text-neutral-700 font-medium'>{state}</h1>
        <p 
        className='text-sm'>Welcome back! Please sign in to continue</p>

        {state !== "Login" && <div 
        className='border px-6 py-2 flex items-center gap-2 rounded-full mt-4 hover:scale-105 transition-all'>
        ...
        <p className='text-sm text-blue-600 my-4 cursor-pointer'>
            ...
            <button className='bg-blue-600 w-full text-white py-2 rounded-full hover:scale-105 transition-all'>
                {state === "Login" ? "Login" : 'Create Account'}</button>

                {state === "Login" ? <p className='mt-4 text-center'> 
